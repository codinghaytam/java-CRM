...
public class AddCommandController implements Initializable {
    ...
    private void AddProduitToController() {
        ...
        try {
            ...
            client = clients.stream().filter(cl-> cl.getLeadId().equals(clientComboBox.getValue())).collect(Collectors.toList()).get(0);
    ...
    public void AddCommande(){
        ...
        try {
            ...
            client=clientDao.selectAll().stream().filter(cl-> cl.getLeadId().equals(clientComboBox.getValue())).collect(Collectors.toList()).get(0);

            LoyaltyCardDaoImpl loyaltyCardDao = new LoyaltyCardDaoImpl();

            loyaltyCard = loyaltyCardDao.findByOwner(client.getLeadId());
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        clientComboBox.setItems(FXCollections.observableArrayList(DisplayClients.stream().map(client-> client.getLeadId()).collect(Collectors.toList())));
    }
}
